  ...
  <div class="w-full h-1/5">
    <header class="flex w-full items-center justify-start py-4">
      <div class="w-[-webkit-fill-available] flex flex-col">
        <!--      Titulo-->
        <p
          class="font-sans-Popping text-3xl flex-col justify-start items-start font-bold text-black-900"
        >
          {{ title }}
          <slot></slot>
        </p>
        <!--      Subtitulo-->
        <p
          class="mt-auto h-1/12 w-4/5 text-xl font-semibold text-primary-seleccionado justify-start mb-auto"
        >
          {{ subtitle }}
        </p>
      </div>
      <div class="flex flex-row h-auto justify-end gap-5">
        <!-- puntaje de usuario -->
        <div
          class="flex h-auto whitespace-nowrap font-semibold text-purple-500 text-lg items-center"
        >
          <svg
            class="mr-3"
            width="20"
